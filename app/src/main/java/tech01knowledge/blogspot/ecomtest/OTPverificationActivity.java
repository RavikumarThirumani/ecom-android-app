...
//                            updateStatus.put("Payment Status", "Paid");
                                            ...
                                            if (task.isSuccessful()) {
                                                ...
                                                FirebaseFirestore.getInstance().collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_ORDERS").document(orderID).set(userOrder)
                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                            ...
                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                if (task.isSuccessful()) {
                                                                    DeliveryActivity.codOrderConfirmation = true;

                                                                    finish();

                                                                } else {
                                                                    Toast.makeText(OTPverificationActivity.this, "Failed Update user order", Toast.LENGTH_LONG).show();
            ...
            public void onErrorResponse(VolleyError error) {
                finish();
                Toast.makeText(getApplicationContext(), "Failed tp send OTP verification code", Toast.LENGTH_LONG).show();
